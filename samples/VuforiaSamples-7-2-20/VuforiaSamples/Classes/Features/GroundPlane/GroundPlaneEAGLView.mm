...
- (id)initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *) app andUIUpdater:(id<GroundPlaneUIControl>) uiUpdater
{
    ...
    if (self) {
        ...
        mAstronaut = [self loadModelNamedWithTransparency:@"astronaut": 1.f];
        mDrone = [self loadModelNamedWithTransparency:@"drone": 1.f];
        mFurniture = [self loadModelNamedWithTransparency:@"chair": 0.5f];
        [self initShaders];
...
- (Modelv3d*) loadModelNamedWithTransparency:(NSString *)modelName : (float) transparency {
                ...
                projectMatrix:(Vuforia::Matrix44F&) projectionMatrix
{
        ...
        for (int i = 0; i < state.getNumTrackableResults(); ++i) {
            ...
            Vuforia::Matrix44F modelViewMatrix = Vuforia::Tool::convertPose2GLMatrix(result->getPose());
             if(result->isOfType(Vuforia::DeviceTrackableResult::getClassType())) {
                ...
                [self setStatusInfoUpdate:result->getStatusInfo()];
    ...
    // Rendering reticle
    if((!render3DReticle && !isAnchorResultAvailable && (mCurrentMode != SAMPLE_APP_FURNITURE_MODE)) || mCurrentMode == SAMPLE_APP_MIDAIR_MODE)
    {
