...
- (void) loadModelNamed:(NSString *)modelName {
    mVikingModel = [[Modelv3d alloc] init];
    [mVikingModel loadModel:modelName];
}
                ...
                projectMatrix:(Vuforia::Matrix44F&) projectionMatrix
{
    ...
    if (mDataset &&
        ...
        float fieldOfView = state.getCameraCalibration()->getFieldOfViewRads().data[1];
    ...
    else
    {
        ...
        if (illumination != nullptr)
        {
            ...
            if (ambientIntensity != Vuforia::Illumination::AMBIENT_INTENSITY_UNAVAILABLE)
            {
                ...
                mAmbientLightIntensity = fmin(0.2f, .03f + (ambientIntensity - 200.0f) / 1300.0f);
            }
        }
...
- (void) renderModelWithProjection: (float*) projectionMatrix withViewMatrix: (float*) viewMatrix andModelMatrix: (float*) modelMatrix
{
    ...
    glVertexAttribPointer(vertexHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)mVikingModel.vertices);
    glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)mVikingModel.normals);
    glVertexAttribPointer(textureCoordHandle, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)mVikingModel.texCoords);
    ...
    // Draw the model
    glDrawArrays(GL_TRIANGLES, 0, (int)mVikingModel.nbVertices);
